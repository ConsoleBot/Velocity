<?php
  ...
  <div class="section-hero h-full pt-[100px] sm:pt-[142px] md:pt-[133px] lg:pt-[176px] xl:pt-[176px] pb-[64px] bg-center bg-cover bg-no-repeat" style="background-image: url('<?php echo esc_url($product_hero_background_overlay); ?>');">
    <div class="text-center h-[372px] sm:h-[525px] md:h-[634px] lg:h-[860px] xl:h-[892px] 2xl:h-[942px]">
      ...
      <h2 data-animate="animate-once animate-fill-both animate-duration-[1000ms]" class="hero-tagline-1 text-[24px] sm:text-[36px] md:text-[48px] lg:text-[64px] font-extrabold tracking-[8px] bg-[linear-gradient(90deg,rgba(70,238,170,0.917647)_0%,rgba(57,215,185,0.958824)_50%,#2CBFC7_100%)] bg-clip-text text-transparent">PROJECT</h2>
      <h1 data-animate="animate-once animate-fill-both animate-duration-[1000ms]" class="hero-tagline-2 mt-[-15px] text-[32px] sm:text-[48px] md:text-[72px] lg:text-[120px] xl:text-[168px] tracking-[0.2px] font-extrabold leading-[80px] sm:leading-[114px] md:leading-[148px] lg:leading-[182px] bg-[linear-gradient(90deg,rgba(70,238,170,0.917647)_0%,rgba(57,215,185,0.958824)_50%,#2CBFC7_100%)] bg-clip-text text-transparent">MANAGEMENT</h1>
    ...
    <div class="bg-[#102321] h-[930px] sm:h-[1024px] lg:h-[1478px] w-full lg:w-[63.4%] pt-[64px] lg:pt-[161px] pl-[16px] lg:pl-[120px] pr-[16px] lg:pr-[80px]">
      ...
      <?php
        ...
        if ($key_feature_photo) {
          echo '<img data-animate="animate-fade-up animate-once animate-fill-both" src="'.esc_url($key_feature_photo).'" class="w-[440px] sm:w-[625px] lg:w-[100%] xl:w-[unset] max-w-[unset] sm:max-w-none absolute top-[300px] lg:top-[30%] left-0 z-[1] pointer-events-none" alt="Screen">';
        }
      ?>
    </div>
    <div class="bg-[#273B39] h-[2105px] sm:h-[1600px] lg:h-auto flex-1 pt-0 lg:pt-[80px] pl-[16px] lg:pl-[50px] pr-[16px] lg:pr-[120px] relative">
      <div class="absolute top-[-200px] sm:top-[-156px] lg:top-[170px] left-0 lg:left-[-75px] z-1">
        <div class="lg:border-b-[2px] border-b-[#CCD6E0] px-[16px] lg:px-[12px] lg:max-h-[1036px] w-[100%] lg:w-[463px] lg:overflow-y-auto mb-[24px] h-full overflow-y-visible">
          <?php
            $page_id = 50;
            ...
            if ($product_key_features) {
              foreach ($product_key_features as $index => $key_feature) {
                echo '<div data-animate="animate-fade-left animate-once animate-fill-both" class="p-[24px] lg:p-[32px] bg-[#F8FAFC] border border-[#CCD6E0] rounded-[16px] mb-[12px] lg:mb-[24px]">';
                  echo '<div class="bg-[#EDF2F7] rounded-full w-[64px] h-[64px] flex items-center justify-center mb-[16px]">';
                    echo '<div style="background-image: url(' . esc_url($key_feature['icon']) . ');" class="bg-cover bg-center w-[32px] h-[32px]"></div>';
                  echo '</div>';
                  echo '<h3 class="text-[#000000] text-[24px] font-extrabold tracking-[0.2px] mb-[8px]">' . esc_html($key_feature['name']) . '</h3>';
                  echo '<p class="font-inter text-[#191919] text-[16px] leading-[157%] tracking-[0.15px]">' . esc_html($key_feature['description']) . '</p>';
                echo '</div>';
              }
            }
          ?>
        </div>
        <p class="hidden lg:block link-button down text-[#FEFEFE] text-[16px] tracking-[0.2px] text-center">
          Scroll to view more
        </p>
      </div>
    </div>
