<?php
  /**
   * Template Name: Transaction Management Template
   */
  get_header('black');
  ?>
  <?php
  ...
  <div class="section-hero h-full pt-[100px] sm:pt-[142px] md:pt-[133px] lg:pt-[176px] xl:pt-[176px] pb-[64px] bg-center bg-cover bg-no-repeat" style="background-image: url('<?php echo esc_url($product_hero_background_overlay); ?>');">
    ...
    <p data-animate="animate-fade-up animate-once animate-fill-both" class="font-inter max-w-[996px] text-center mx-auto text-[#FEFEFE] text-[16px] md:text-[20px] leading-[157%] tracking-[0.15px]">
      Designed specifically for the complexities of commercial real estate transactions. Our application<br> 
      ...
      <div class="overflow-hidden relative lg:flex justify-end items-center h-[500px] lg:h-[629px] mb-[30px] lg:mb-[79px]">
        <?php
          ...
          if ($key_feature_photo) {
            echo '<img data-animate="animate-fade-up animate-once animate-fill-both" src="'.esc_url($key_feature_photo).'" class="max-w-[unset] lg:max-w-[100%] absolute pointer-events-none top-[232px] lg:top-[0] w-[439px] lg:w-[88.2%] left-[-30px] lg:left-[-45px]" alt="Screen">';
          }
        ?>
        <div class="max-w-[486px]">
          <h2 data-animate="animate-fade-right animate-once animate-fill-both" class="text-[#FEFEFE] text-[32px] lg:text-[56px] font-extrabold tracking-[0.2px] mb-[16px]">Key  Feature</h2>
          <p data-animate="animate-fade-up animate-once animate-fill-both" class="font-inter text-[16px] text-[#FEFEFE] leading-[157%] tracking-[0.15px]">Our platform simplifies property management with an intuitive dashboard, automated leases and payments, real-time maintenance tracking, and powerful reporting tools.</p>
        </div>
      </div>
      <div class="mb-[76px] grid grid-cols-[repeat(auto-fill,minmax(275px,1fr))] gap-[24px] flex-wrap">
        <?php
          $page_id = 42;
          ...
          if ($product_key_features) {
            foreach ($product_key_features as $index => $key_feature) {
              echo '<div data-animate="animate-fade-left animate-once animate-fill-both" class="p-[32px] bg-[#141817] border border-[#333739] rounded-[16px]">';
                echo '<h3 class="text-[#FEFEFE] text-[24px] font-extrabold tracking-[0.2px] mb-[8px]">' . esc_html($key_feature['name']) . '</h3>';
          ...
          <div>
            <div class="relative tabs mt-10">
              <button class="tab-prev lg:hidden absolute left-0 z-10 top-[7px] px-2 py-1">
                <i class="fa-solid fa-angle-left text-[#FEFEFE]"></i>
              </button>
                  ...
                  if ($product_features) {
                    ...
                    foreach ($product_features as $index => $feature) {
                      ...
                      echo '<button 
                        ...
                        class="tab-btn min-w-fit lg:min-w-unset focus:outline-none cursor-pointer p-[16px] lg:p-[24px] pt-[10px] text-[16px] lg:text-[20px] text-[#4B5052] aria-selected:text-[#FEFEFE] font-semibold tracking-[0.2px] border-b-[2px] border-b-[#4B5052] aria-selected:border-b-[#FEFEFE]">'
                ...
                <?php
                  if ($product_features) {
                    foreach ($product_features as $index => $feature) {
                      ...
                      echo '<p data-animate="text-[#FEFEFE] animate-fade-up animate-once animate-fill-both" id="' . $panel_id . '" role="tabpanel" class="mx-auto max-w-[800px] text-center ' . $hidden_class . '">'
              ...
              <button class="tab-next lg:hidden absolute right-0 z-10 top-[7px] px-2 py-1">
                <i class="fa-solid fa-angle-right text-[#FEFEFE]"></i>
              </button>
            </div>
          </div>
        </div>
  ...
  <section class="bg-[#252829]">
    <div class="container px-0! md:px-[16]!">
      <div class="pb-[173px] sm:pb-[151px] lg:pb-[151px] lg:py-[160px] flex justify-end">
        <div class="flex justify-center items-center md:block py-[64px] pb-[320px] md:pt-[56px] px-[56px] md:pl-[0] md:pr-[56px] lg:pr-[78px] mr-[0] lg:mr-[102px] border-2 lg:border-[#FEFEFE4D] rounded-0 md:rounded-[16px] h-full md:h-[327px] w-full md:w-[914px] bg-[#141817] relative">
          <?php
            $page_id = 69;
  ...
  <section class="bg-[linear-gradient(180deg,#252829_0%,#212121_100%)] relative">
    <?php
      $page_id = 69;
    ...
    <div class="container">
      <div class="py-[64px] lg:py-[124px] max-w-[841px] mx-auto flex flex-col items-center">
        ...
        <a data-animate="animate-fade-up animate-once animate-fill-both" href="" class="text-[16px] text-[#191919] py-[12px] px-[24px] bg-[#FEFEFE] rounded-[100px] tracking-[0.25px] font-bold z-1">Book a Demo</a>
      </div>
    </div>
  </section>
<?php get_footer(); ?>
